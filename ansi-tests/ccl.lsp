...
(deftest ccl.issue#167
    (values
     (= (/ #C(-751132490354605/1925220047711698 30102855149/57025656074))
        #C(-587824275525576799626201439150548126508540663569005/649184300447915346652175104735572636950908622715613 -795332524938176444532866668598800193038885551652913/649184300447915346652175104735572636950908622715613)
        )
     (= (* 27642600875762413724933214567114845097636269322/124861921458285773590870637249488964684495255 1/2)
        13821300437881206862466607283557422548818134661/124861921458285773590870637249488964684495255))
  t t)

(deftest ccl.issue#176
    (values (= (+ 9223372036854775807/9223372036854775808 -9223372036854775808/9223372036854775807)
...
;;; bad register targeting with 2D double-float arrays
(deftest ccl.issue#335
  (let ((a (make-array '(2 2) :element-type 'double-float))
    ...
    (flet ((init (a b)
             (dotimes (i 4)
               (setf
                ;; 1 2 3 4
                (row-major-aref a i) (float (+ 1 i) 0d0)
                ;; 5 6 7 8
                (row-major-aref b i) (float (+ 5 i) 0d0))))
           (f1 (a b)
             (declare (type (array double-float (* *)) a b))
             (declare (optimize (speed 3) (safety 0)))
             (incf (aref a 0 0) (aref b 0 0)))
           (f1-simple (a b)
           ...
           (f2 (a b)
             ...
             (loop for j from 0 to 0 do
               (loop for k from 0 to 0 do
                 (setf (aref a 0 j)
                       (aref b k j))))))
      (values (progn (init a b)
                     (f1 a b)
                     (eql 6.0d0 (aref a 0 0)))
              (progn (init a b)
                     (f1-simple a b)
                     (eql 6.0d0 (aref a 0 0)))
              (handler-case (progn (init a b)
                                   (f2 a b)
